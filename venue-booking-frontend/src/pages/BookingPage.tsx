...
type Venue = '大會堂' | '康樂廳' | '其它教室' | '慈助會教室' | '廚房'
...
function tpeDow(d: Date) { return new Date(`${tpeDateKey(d)}T12:00:00+08:00`).getUTCDay() }
...
/** 將任意開始時間調整到「可申請窗口」並回傳狀態（固定 3.5h） */
...
export default function BookingPage() {
  ...
  const repeatPreview: PreviewItem[] = useMemo(() => {
    ...
    for (const iso of startsFromPicker) {
      ...
      if ((adj as any).status === 'invalid') items.push({ date: s, status: 'invalid' })
      else if ((adj as any).status === 'sunday') items.push({ date: s, status: 'skip_sun' })
      else items.push({ date: s, start: (adj as any).start, end: (adj as any).end, status: (adj as any).cut ? 'cut' : 'ok' })
    }
  ...
  <option value="其它教室">其它教室</option>
        ...
        {mode === 'single' ? (
          ...
          <>
            ...
            <div className="border rounded p-3">
              <RepeatTwoWeeksFlexiblePicker firstStartISO={firstStartISO} initialEnabled={true} initialPerWeek={true} onStartsPreview={setStartsFromPicker} />
            </div>
      ...
      <p className="text-xs text-slate-500">
        規範以台北時間計：每日最早 07:00；週一/週三最晚 18:00；其他至 21:30；週日禁用。每一日最多 3.5 小時。重複日期最長 2 週。
      </p>
    </div>
  )
}
