...
// 3) CORS：允許跨網域 + 攜帶 Cookie（credentials）
...
const corsOptions: cors.CorsOptions = {
  origin(origin, cb) {
    if (!origin) return cb(null, true) // 同源/伺服器到伺服器/Postman
...
if (process.env.DATABASE_URL) {
  ...
  store = new PgStore({
    pool: pgPool,
    tableName: 'session',
    createTableIfMissing: true, // ★ 沒有 session 表時自動建立
  })
  console.log('[api] session store: Postgres')
} else {
  console.warn('[api] session store: MemoryStore（未設 DATABASE_URL，僅適合開發）')
}
...
// debug: 觀察 terms 表（依你程式的 user_id/accepted_at 欄位）
