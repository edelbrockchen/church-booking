...
const CreateBody = z.object({
  start: z.string().min(10), // ISO string；伺服器轉 Date 檢查
...
router.post('/', async (req, res) => {
  ...
  // 讓 TypeScript 看到已經排除 null（使用區域別名）
  const p = pool
